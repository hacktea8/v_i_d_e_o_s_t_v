<div class="foot">
 <p class="footermenu">
  <a href="/faq.html">使用帮助</a> |
  <a href="#" title="给我留言" class="color">给我留言</a> |
  <a href="/allmovie.html">最新更新</a> |
  <a href="/top.html" class="color">排 行 榜</a> |
  <a href="/search.php?searchtype=5">索 引 库</a>
 </p>
 <p class="copyrighten">{pipicms:copyright}</p>
 <p class="copyrighten2">Copyright &copy;  2008-<?php echo date('Y');?> <a class="color" href="<?php echo $siteurl;?>"><?php echo $web_title,$domain_url;?></a></p>
 <p class="copyrightzh">联系我们：xxx@xxx.com</p>
 <p class="dn">{pipicms:sitevisitjs}</p>
</div>
<script type="text/javascript">
function _Userlogin(){
  var timer=null;
  var _hide=function(){
    $('.iconList').hide();$('.dropMenu').hide();}
  var init=function(){
    $('#user_login').mouseout(function(){
      timer=setTimeout(_hide,500);});
    $('#user_login').mouseover(function(){
     clearTimeout(timer);
     if($('.iconList').is(":visible") || $('.dropMenu').is(":visible")){
       return false;}
     $.get('/<?php echo $_c;?>/isUserInfo/',function(data){
       if(data.status==1){
         $('.iconList').show();$('.dropMenu').hide();
       }else{
         $('.iconList').hide();$('.dropMenu').show();}
      },"json");});}
  init();}
_Userlogin();
</script>
</body>
</html>
