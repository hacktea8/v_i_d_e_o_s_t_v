...
function submitComment(id){
	if($("username").value.length<1){alert('请填写昵称');return false;}
	if($("msg").value.length<1){alert('请填写内容');return false;}
	ajax.postf(
		$("f_comment"),
...
function leaveWord(){
	...
	ajax.postf(
		$("f_leaveword"),
		function(obj){if(obj.responseText=="ok"){viewLeaveWordList(1);alert('留言成功，多谢支持！');$("m_content").value='';}else if(obj.responseText=="haveleave"){alert('小样儿你手也太快了，歇会儿再来留言吧！');}else{alert('发生错误');}}
	);
}
